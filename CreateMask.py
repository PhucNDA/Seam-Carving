...
def getMaskObject(im):
    ...
    def createMask(img):
        numberImageMask = len(os.listdir('img-mask')) + 1
        ...
        cv2.imwrite(".\img-mask\img-mask" + str(numberImageMask) + ".png", img)
        cv2.waitKey(0)
        return img

    numberImageDraw = len(os.listdir('.\img-draw')) + 1
    ...
    while(1):
        ...
        cv2.imwrite(".\img-draw\img-draw" + str(numberImageDraw) + ".png", im)
        ...
        if k == 27: break
            ...
            if size_mouse >= 100:
                size_mouse = 100
                print(f"Upper limit, Increate size mouse = {size_mouse}")
            else:
